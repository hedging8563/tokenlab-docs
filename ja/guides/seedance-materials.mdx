...
素材ライブラリでは 30 日間の非アクティブ削除ルールが適用されます。アップロードから 30 日以上経過し、過去 30 日間に生成リクエストで一度も参照されていない素材は自動的に削除されます。削除後は復元できません。

- 生成リクエストで素材 ID（`material_asset_id`、`material_asset_ids` または `asset://<id>`）を参照すると使用とみなされ、その素材はこのルールでは削除されません。
- 重要な素材は事前にソースファイルをバックアップしてください。削除された素材を再度利用するには再インポートが必要で、新しい素材 ID が発行されます。
...
TokenLab は、素材またはその素材グループを削除するまで、組織の素材ライブラリに素材を保持します。長期間参照されていない素材は、上記の「素材の有効期限」ルールにより自動的に削除されます。
