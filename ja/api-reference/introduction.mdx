...
`GET /v1/models`、`GET /v1/models/{model}`、`GET /v1/pricing` は公開されており、キーは不要です。Anthropic Messages は `x-api-key` も、Gemini は Bearer に加えて `x-goog-api-key` または `?key=` も受け付けます。`/v1/management/*` には管理トークン（`mt-...`）が必要です。
