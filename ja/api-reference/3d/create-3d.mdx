---
title: "3Dモデルの作成"
openapi: "/openapi/ja.json POST /v1/3d/generations"
...
オプションのフィールドを送信する前に、`GET /v1/models/{model}` でモデルが受け付けるパラメータを確認してください。例では `model` と `prompt` のみを送信します。
...
<ParamField body="format" type="string">
  出力形式。対応状況と指定可能な値はモデルによって異なります。
</ParamField>

<ParamField body="quality" type="string">
  品質設定。対応状況と指定可能な値はモデルによって異なります。
</ParamField>
