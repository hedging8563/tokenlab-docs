...
openapi: "/openapi.json GET /v1/management/api-keys/{keyId}/usage"
...
Return paginated usage records for one API key.
...
- Settled records can include `billing_transaction_id`. Save it with `request_id` when you need to match a charge to a request.
...
<ResponseField name="items[].requested_policy" type="string | null">`auto`, `verified`, `official`, `null`. Requested delivery policy, when recorded.</ResponseField>
<ResponseField name="items[].resolved_tier" type="string | null">`verified`, `official`, `null`. Delivery tier used, when recorded.</ResponseField>
<ResponseField name="items[].charged_price_basis" type="object | null">Applied price and billable quantity, when recorded.</ResponseField>
