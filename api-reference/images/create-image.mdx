...
- `nano-banana-2` supports `aspect_ratio` and `resolution` (`1k`, `2k`, `4k`) for both text-to-image and image-to-image generation.
...
Query `GET /v1/models?recommended_for=image` or use the [Models page](https://tokenlab.sh/en/models) for current availability, capabilities, and prices.
