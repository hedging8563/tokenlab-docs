...
<Info>
Seedance can save request images as reusable materials in the current Workspace. These fields also accept TokenLab material URIs such as `asset://asset-YYYYMMDDHHMMSS-xxxxx`. If a material is not ready within 60 seconds, the API returns `409 seedance_material_preparing` with `auto_material_asset_ids`; retry after the materials become `ACTIVE`.
</Info>
