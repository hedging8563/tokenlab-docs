...
<Note>
Use `GET /v1/models?recommended_for=3d` to get current 3D models.
</Note>
...
Before sending optional fields, check `GET /v1/models/{model}` for the model’s accepted parameters. The example uses only `model` and `prompt`.
...
<ParamField body="format" type="string">
  Output format; support and allowed values depend on the selected model.
</ParamField>

<ParamField body="quality" type="string">
  Quality setting; support and allowed values depend on the selected model.
</ParamField>
...
<ParamField body="seed" type="integer">
  Integer seed, when supported by the selected model.
</ParamField>
