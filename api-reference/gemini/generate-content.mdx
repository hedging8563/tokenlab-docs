...
This endpoint uses Gemini request and response formats. It does not convert requests to Chat Completions. Google ProtoJSON lowerCamelCase and proto `snake_case` field names are both accepted. Optional field support depends on the selected model.
...
<ParamField body="contents" type="array" required>
  ...
  File and Cache IDs created by TokenLab can be used only in their owning Workspace.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  ...
  - `candidateCount` (integer): Requested candidate count; support depends on the model.
  - `responseModalities` (array): Requested output modalities for compatible models.
