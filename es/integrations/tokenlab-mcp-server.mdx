...
<CardGroup cols={2}>
  <Card title="Instalar en Cursor" icon="arrow-pointer" href="https://cursor.com/en/install-mcp?name=tokenlab&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkB0b2tlbmxhYmFpL21jcC1zZXJ2ZXJAMC42LjIwIl0sImVudiI6eyJUT0tFTkxBQl9NQ1BfVE9PTF9QUk9GSUxFIjoiY2F0YWxvZyJ9fQ%3D%3D">
...
<Tabs>
      ...
      <Tab title="Claude Desktop">
        Añade al objeto estándar `mcpServers` de `claude_desktop_config.json` el comando `npx` y los argumentos `-y` y `@tokenlabai/mcp-server@0.6.20`. Si solo necesitas herramientas públicas, configura `TOKENLAB_MCP_TOOL_PROFILE=catalog`.
      </Tab>
...
En cualquier cliente stdio local usa el nombre `tokenlab`, el comando `npx`, los argumentos separados `-y` y `@tokenlabai/mcp-server@0.6.20`, y deja vacío el directorio de trabajo. Usa `@tokenlabai/mcp-server@0.6.20` para fijar la versión verificada actualmente.
...
<AccordionGroup>
  <Accordion title="El cliente no puede iniciar el servidor">
    Confirma que Node.js sea al menos 18.17 y que el cliente encuentre `npx`. Ejecuta `npx -y @tokenlabai/mcp-server@0.6.20` en una terminal para mostrar errores de npm o de red. Un inicio stdio correcto espera silenciosamente mensajes MCP; termina con **Ctrl+C** después de comprobarlo.
  </Accordion>
  ...
  <Accordion title="¿Cómo fijo o actualizo la versión del paquete?">
    Usa `@tokenlabai/mcp-server@0.6.20` para fijar la versión actualmente verificada. Para cambiarla, actualiza el argumento del paquete, reinicia el servidor y revisa el repositorio público antes de adoptar una versión más reciente.
  </Accordion>
</AccordionGroup>
