...
En cualquier cliente stdio local usa el nombre `tokenlab`, el comando `npx`, los argumentos separados `-y` y `@tokenlabai/mcp-server`, y deja vacío el directorio de trabajo. Usa `@tokenlabai/mcp-server@0.6.17` para fijar la versión verificada actualmente.
