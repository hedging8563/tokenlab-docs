...
<CardGroup cols={2}>
  <Card title="Instalar en Cursor" icon="arrow-pointer" href="https://cursor.com/en/install-mcp?name=tokenlab&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkB0b2tlbmxhYmFpL21jcC1zZXJ2ZXIiXSwiZW52Ijp7IlRPS0VOTEFCX01DUF9UT09MX1BST0ZJTEUiOiJjYXRhbG9nIn19">
    Añade el perfil público `catalog` a Cursor con un clic
  </Card>
  <Card title="Instalar en VS Code" icon="code" href="https://vscode.dev/redirect/mcp/install?name=tokenlab&config=%7B%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40tokenlabai%2Fmcp-server%22%5D%2C%22env%22%3A%7B%22TOKENLAB_MCP_TOOL_PROFILE%22%3A%22catalog%22%7D%7D">
  ...
  <Step title="Configurar el cliente">
    ...
    <Tabs>
      ...
      <Tab title="Codex">
        ...
        # Perfil core predeterminado con clave de API
        codex mcp add --env TOKENLAB_API_KEY=your-tokenlab-api-key tokenlab -- npx -y @tokenlabai/mcp-server
        ```
      </Tab>

      <Tab title="Claude Desktop">
        Añade al objeto estándar `mcpServers` de `claude_desktop_config.json` el comando `npx` y los argumentos `-y` y `@tokenlabai/mcp-server`. Si solo necesitas herramientas públicas, configura `TOKENLAB_MCP_TOOL_PROFILE=catalog`.
      </Tab>

      <Tab title="Otros clientes">
        Cline CLI puede instalar directamente el servidor stdio publicado.
...
En cualquier cliente stdio local usa el nombre `tokenlab`, el comando `npx`, los argumentos separados `-y` y `@tokenlabai/mcp-server`, y deja vacío el directorio de trabajo. Usa `@tokenlabai/mcp-server@0.6.19` para fijar la versión verificada actualmente.
