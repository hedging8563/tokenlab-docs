...
`GET /v1/models`, `GET /v1/models/{model}` y `GET /v1/pricing` son públicos y no requieren clave. Anthropic Messages también acepta `x-api-key`; Gemini acepta `x-goog-api-key` o `?key=` además de Bearer. `/v1/management/*` requiere un token de gestión (`mt-...`).
