---
title: "Crear imagen"
openapi: "/openapi/es.json POST /v1/images/generations"
...
`input_fidelity` no forma parte de los campos admitidos actual de TokenLab para `gpt-image-2`; omítelo o la solicitud devolverá `400 unsupported_parameter`.
...
- `nano-banana-2` admite `aspect_ratio` y `resolution` (`1k`, `2k`, `4k`) tanto para text-to-image como para image-to-image.
...
<ParamField body="resolution" type="string">
  La resolución depende del modelo. `gemini-3.1-flash-image`, `gemini-3-pro-image`, `nano-banana-2` y `nano-banana-pro` admiten `1k`, `2k` y `4k`; los modelos de imagen Grok Imagine admiten `1k` y `2k`. Envía este campo solo si el modelo y la operación lo admiten explícitamente.
</ParamField>
...
Consulta `GET /v1/models?recommended_for=image` para ver los modelos de imagen, capacidades y precios actuales.
...
Para los modelos de imagen, comprueba siempre si la respuesta contiene `status: "pending"` / `status: "processing"`:
