...
https://docs.tokenlab.sh/integrations/coding-agent-skill
...
Open [Console → API keys](https://tokenlab.sh/dashboard/api?tab=keys&createKey=1&docsTopic=codingAgents), create a key, and store it in an environment variable. A TokenLab API key starts with `sk-`.
...
For API or account questions, use [in-app Support](https://tokenlab.sh/dashboard/support); review the preview and submit explicitly. For a particular request, start with [Requests](https://tokenlab.sh/dashboard/runs) and follow the [investigation guide](/guides/observability-troubleshooting). Report defects in the Skill itself through [GitHub Issues](https://github.com/hedging8563/tokenlab-skills/issues).
