...
Use a current chat model ID from the [Models page](https://tokenlab.sh/en/models). Cursor's BYOK settings do not make every Cursor-managed feature available through a custom API key.
...
<AccordionGroup>
  ...
  <Accordion title="Model unsupported in BYOK mode">
    Custom API keys only work with standard chat models. Choose a current chat model from the TokenLab Models page.
  </Accordion>
...
Model IDs and prices change. Choose a current chat model from the [Models page](https://tokenlab.sh/en/models), then compare it on your own project.
