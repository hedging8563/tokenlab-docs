...
- The [TokenLab Skill](/integrations/coding-agent-skill) installs integration instructions with `npx skills add`; it does not start an MCP server.
...
https://docs.tokenlab.sh/integrations/tokenlab-mcp-server
...
<AccordionGroup>
  <Accordion title="The client cannot start the server">
    Run `node --version`, `npx --version`, and `npx -y @tokenlabai/mcp-server@0.6.19` in a terminal. A successful stdio server waits silently for MCP messages; press **Ctrl+C** after confirming it starts.
  </Accordion>
