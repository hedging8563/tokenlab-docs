...
<Tabs>
  <Tab title="Claude Code">
    ...
    claude mcp add \
      ...
      --scope user \
      tokenlab -- \
      npx -y @tokenlabai/mcp-server@0.6.22
    ```
  ...
  <Tab title="Other clients">
    ...
    | --- | --- |
    | Command | `npx` |
    | Arguments | `-y`, `@tokenlabai/mcp-server@0.6.22` |
...
<AccordionGroup>
  <Accordion title="The client cannot start the server">
    Run `node --version`, `npx --version`, and `npx -y @tokenlabai/mcp-server@0.6.22` in a terminal. A successful stdio server waits silently for MCP messages; press **Ctrl+C** after confirming it starts.
  </Accordion>
