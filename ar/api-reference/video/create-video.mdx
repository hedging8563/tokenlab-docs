---
title: "إنشاء فيديو"
openapi: "/openapi/ar.json POST /v1/videos/generations"
...
<ParamField body="material_asset_id" type="string">
  معرف مادة Seedance من TokenLab المعاد من [إنشاء مادة](/ar/api-reference/video/create-material-asset). استخدمه بعد أن تصبح المادة `ACTIVE` مع نماذج Seedance التي يمكنها استخدام مكتبة مواد TokenLab.
</ParamField>
...
<Info>
تُستخدم روابط الصور العادية كمدخلات ولا تنشئ مواد قابلة لإعادة الاستخدام تلقائيًا. أنشئ المواد عبر واجهة المواد ثم استخدم معرفات TokenLab أو روابط `asset://asset-YYYYMMDDHHMMSS-xxxxx`. إذا أعادت المواد المحددة `409 seedance_material_preparing`، فتحقق من `inactive_asset_ids` وأعد المحاولة بعد أن تصبح المواد `ACTIVE`.
</Info>
...
<ParamField body="kling_elements" type="array">
  استخدم `kling_elements` فقط إذا أدرجته التفاصيل العامة الحالية للنموذج. أرسل صورًا و1–3 عناصر، لكل منها `name` و`description` اختياري و2–4 روابط `element_input_urls`، وأشر إليها بصيغة `@name` في `prompt`. لا تجمعها مع `output_audio=true`.
</ParamField>
...
في video-to-video مع `grok-imagine-video`، أرسل رابط HTTPS عام بصيغة `.mp4` في `video_url`.  يمكنك ضبط `resolution` على `480p` أو `720p`؛ ولا يقبل مسار التحرير هذا `duration` أو `aspect_ratio`.
