...
```text
Lies diese Anleitung und wähle für meine Aufgabe MCP-Katalogwerkzeuge oder den TokenLab Skill:
https://docs.tokenlab.sh/de/integrations/tokenlab-mcp-server
...
<CardGroup cols={2}>
  <Card title="In Cursor installieren" icon="arrow-pointer" href="https://cursor.com/en/install-mcp?name=tokenlab&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkB0b2tlbmxhYmFpL21jcC1zZXJ2ZXJAMC42LjIwIl0sImVudiI6eyJUT0tFTkxBQl9NQ1BfVE9PTF9QUk9GSUxFIjoiY2F0YWxvZyJ9fQ%3D%3D">
    Das öffentliche Profil `catalog` mit einem Klick in Cursor hinzufügen
  </Card>
  <Card title="In VS Code installieren" icon="code" href="https://vscode.dev/redirect/mcp/install?name=tokenlab&config=%7B%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40tokenlabai%2Fmcp-server%400.6.20%22%5D%2C%22env%22%3A%7B%22TOKENLAB_MCP_TOOL_PROFILE%22%3A%22catalog%22%7D%7D">
...
<Tabs>
      ...
      <Tab title="Claude Desktop">
        Ergänzen Sie die Standardkonfiguration `mcpServers` in `claude_desktop_config.json` mit dem Befehl `npx` und den Argumenten `-y` sowie `@tokenlabai/mcp-server@0.6.20`. Für ausschließlich öffentliche Tools setzen Sie `TOKENLAB_MCP_TOOL_PROFILE=catalog`.
      </Tab>
...
Verwenden Sie für lokale stdio-Clients den Servernamen `tokenlab`, den Befehl `npx`, die getrennten Argumente `-y` und `@tokenlabai/mcp-server@0.6.20` sowie kein Arbeitsverzeichnis. Mit `@tokenlabai/mcp-server@0.6.20` fixieren Sie die aktuell geprüfte Version.
...
<AccordionGroup>
  <Accordion title="Der Client kann den Server nicht starten">
    Stellen Sie sicher, dass Node.js mindestens Version 18.17 ist und der Client `npx` findet. Führen Sie `npx -y @tokenlabai/mcp-server@0.6.20` in einem Terminal aus, um npm- oder Netzwerkfehler anzuzeigen. Ein erfolgreicher stdio-Start wartet still auf MCP-Nachrichten; beenden Sie ihn nach der Prüfung mit **Ctrl+C**.
  </Accordion>
  ...
  <Accordion title="Wie fixiere oder aktualisiere ich die Paketversion?">
    Mit `@tokenlabai/mcp-server@0.6.20` fixieren Sie die aktuell geprüfte Version. Ändern Sie für ein Update das Paketargument, starten Sie den Server neu und prüfen Sie vor der Übernahme eines neueren Releases das öffentliche Repository.
  </Accordion>
</AccordionGroup>
