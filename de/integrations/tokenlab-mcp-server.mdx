...
Verwenden Sie für lokale stdio-Clients den Servernamen `tokenlab`, den Befehl `npx`, die getrennten Argumente `-y` und `@tokenlabai/mcp-server` sowie kein Arbeitsverzeichnis. Mit `@tokenlabai/mcp-server@0.6.17` fixieren Sie die aktuell geprüfte Version.
