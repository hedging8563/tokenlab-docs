...
通过 [Models API](/zh/api-reference/models/list-models) 获取当前可用的 Marble 模型。该接口暂不支持 `marble-1.0-draft`。
...
World Labs 按 credits 计费。创建任务时可能暂扣最高预估金额，完成后根据 `cost.total_credits` 记录实际费用。标准 Marble 请求最多 1,600 credits，`marble-1.1-plus` 最多 3,100 credits。

`marble-1.0-draft` 和独立 pano / depth 工具当前不可用。
