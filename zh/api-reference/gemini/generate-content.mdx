---
title: "生成内容"
openapi: "/openapi/zh.json POST /v1beta/models/{model}:generateContent"
description: "使用 Google Gemini API 格式生成内容"
---

使用 Gemini Models API 返回的模型，以 Gemini 请求和响应格式调用。支持 ProtoJSON lowerCamelCase 和对应的 proto snake_case 字段名；可选字段和取值以所选模型为准。
...
<ParamField body="contents" type="array" required>
  对话内容，包含 `parts` 和可选的 `role`。媒体字段使用 `inlineData` / `inline_data` 或 `fileData` / `file_data`。TokenLab 创建的 Files 和 Cache ID 只能在所属工作区使用。
</ParamField>
...
<ParamField body="generationConfig" type="object">
  生成设置包括 `temperature`、`topP`、`topK`、`maxOutputTokens`、`stopSequences`、`candidateCount`、`responseModalities`、`responseMimeType`、`responseSchema` 和 `thinkingConfig`，也支持对应的 snake_case 名称。取值范围与组合以所选模型为准。
</ParamField>
