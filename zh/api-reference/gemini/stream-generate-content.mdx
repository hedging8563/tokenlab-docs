...
这是 `generateContent` 的 SSE 版本。中间事件可能只有 metadata，也可能暂时没有 `finishReason`；流会在自然 EOF 时结束，不会附加 Chat Completions 的 `[DONE]`。
## 路径参数
...
`generationConfig.candidateCount`、工具组合和其他可选字段是否可用，以所选模型为准。
