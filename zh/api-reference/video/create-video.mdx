...
<Info>
Seedance 可把请求中的图片保存为当前工作区的可复用素材，也接受 `asset://asset-YYYYMMDDHHMMSS-xxxxx` 形式的 TokenLab 素材 URI。若素材在 60 秒内尚未就绪，API 会返回 `409 seedance_material_preparing` 和 `auto_material_asset_ids`；素材状态变为 `ACTIVE` 后即可重试。
</Info>
