...
openapi: "/openapi/zh.json POST /api/v3"
description: "通过火山兼容 Action 管理 Seedance 素材和素材组。"
---

这些接口支持下列素材 Action 和字段，使用 TokenLab Bearer 认证及 `ResponseMetadata + Result` 响应结构。
...
`ProjectName` 默认为 `default`。同一组素材的创建、查询、更新和删除请求需要使用相同的值。
...
| `ListAssetGroups` | 列出项目内的素材组 | `Filter.GroupIds`; `Filter.GroupType`; `Filter.Name`; `PageNumber`; `PageSize`; `SortBy`; `SortOrder`; `ProjectName` |
...
| `ListAssets` | 列出项目内的素材 | `Filter.GroupIds`; `Filter.GroupType`; `Filter.Statuses`; `Filter.Name`; `PageNumber`; `PageSize`; `SortBy`; `SortOrder`; `ProjectName` |
...
请使用 TokenLab 素材 ID。`CreateAsset` 在处理完成前返回 `Result.Id`；轮询 `GetAsset`，直到 `Result.Status` 为 `Active` 或 `Failed`。`ListAssets` 还支持 `SortBy: "GroupId"`。
...
错误码和消息位于 `ResponseMetadata.Error`。
...
也可以使用 snake_case 的 `/v1/videos/assets*` REST 接口。完整用法见 [Seedance 素材与真人验证](/zh/guides/seedance-materials)。
