...
<ParamField path="id" type="string" required>
  创建视频时返回的任务 ID。`id` 和 `task_id` 指向同一个任务。
</ParamField>

<Info>
如果创建响应中包含 `poll_url`，直接请求这个地址即可。

找不到任务时，TokenLab 会返回 `async_task_not_found` 和消息 `Task not found or no longer available.`
</Info>
