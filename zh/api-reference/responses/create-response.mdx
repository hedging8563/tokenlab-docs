...
模型详情中的 `accepted_request_formats` 包含 `responses` 时，可以使用该接口。请求和响应保持 Responses API 格式，不会转换成 Chat Completions。
## 请求体
...
<ParamField body="text" type="object">
  文本输出设置。`text.format` 是否可用取决于所选模型。
</ParamField>
