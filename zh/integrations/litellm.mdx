---
title: "LiteLLM"
description: "把 LiteLLM 连接到 TokenLab"
---

## 概述

LiteLLM 可以通过两种方式使用 TokenLab：

- 把 TokenLab 添加为 **OpenAI 兼容服务**
- 团队还需要 LiteLLM 的虚拟密钥、别名或日志时，使用 LiteLLM Gateway

使用 Chat Completions 时，在 LiteLLM 的 **custom OpenAI / OpenAI 兼容**配置中把 `api_base` 设为 `https://api.tokenlab.sh/v1`。

<Note>
应用需要 Claude Messages 或 Gemini 字段时，请使用能够保留对应 API 格式的集成。
</Note>
