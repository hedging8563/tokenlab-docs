...
備份並合併設定，不覆蓋既有帳戶、權限或無關環境變數。`ANTHROPIC_API_KEY` 傳送 `X-Api-Key`，`ANTHROPIC_AUTH_TOKEN` 傳送 Bearer token，請明確選擇認證方式。啟動後以 `/status` 檢查模型與設定來源，這不代表付費請求成功。簡短付費測試後核對回覆與 TokenLab 記錄。還原時僅撤銷新增設定。參閱[設定優先順序](https://code.claude.com/docs/en/settings)。
