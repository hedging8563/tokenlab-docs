---
title: "建立 3D 模型"
openapi: "/openapi/zh-Hant.json POST /v1/3d/generations"
...
傳送可選欄位前，請透過 `GET /v1/models/{model}` 查看該模型接受的參數。範例只傳送 `model` 和 `prompt`。
...
<ParamField body="format" type="string">
  輸出格式；是否支援及允許值取決於所選模型。
</ParamField>

<ParamField body="quality" type="string">
  品質設定；是否支援及允許值取決於所選模型。
</ParamField>
