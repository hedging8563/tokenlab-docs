...
Model istekleri TokenLab API anahtarı kullanır. Standart kimlik doğrulama başlığı şöyledir:

```bash
Authorization: Bearer sk-your-api-key
```

`GET /v1/models`, `GET /v1/models/{model}` ve `GET /v1/pricing` herkese açıktır ve anahtar gerektirmez. Anthropic Messages ayrıca `x-api-key`, Gemini ise Bearer dışında `x-goog-api-key` veya `?key=` kabul eder. `/v1/management/*` bir yönetim tokenı (`mt-...`) gerektirir.

API anahtarınızı [Kontrol Paneli](https://tokenlab.sh/dashboard) üzerinden alın.

<Note>
Üretim istekleri `X-TokenLab-Delivery-Policy: auto | verified | official` kabul eder. Başlık, API anahtarı ayarından; anahtar ayarı da çalışma alanı varsayılanından önceliklidir. `auto` önce TokenLab Verified, gerektiğinde Official kullanır; ücret, isteği tamamlayan yönteme göre belirlenir. `verified` TokenLab fiyatlarını, `official` üreticinin açık fiyatlarını esas alır; geçerli ücret TokenLab’da gösterilendir. Realtime anahtar veya çalışma alanı ayarını kullanır, sorgu ile değiştirilemez. Geçersiz başlık `400`, kullanılamayan yöntem ise `503 delivery_tier_unavailable` ve istek kimliği döndürür.
</Note>
...
<Info>
Görsel modeller tamamlanmış bir görsel veya asenkron görev döndürebilir. Yanıtta `poll_url` varsa görevi sorgulamak için bu URL’yi kullanın.
</Info>
...
Her uç nokta kendi API biçimini korur. Aşağıdaki başarı ve hata örnekleri Chat Completions biçimindedir.
...
TokenLab, herkese açık yanıt gövdelerinde sağlayıcı, kanal, ilke veya kimlik bilgisi ayrıntılarını göstermez. `_routing` veya diğer dahili yönlendirme alanlarına herkese açık API sözleşmesinin bir parçası olarak güvenmeyin.

Hata ayıklama ve destek için, mevcut olduklarında herkese açık yanıt başlıklarını kullanın:

| Başlık | Açıklama |
|-------|-------------|
| `X-Routing-Time-MS` | Rota seçimi süresi, mevcutsa |
| `X-Request-ID` | Destek ve hata ayıklama için istek tanımlayıcısı, mevcutsa |
| `X-Task-ID` | Görev tabanlı yanıtlar için herkese açık asenkron görev tanımlayıcısı, mevcutsa |
| `X-Billing-Transaction-ID` | Nihai faturalandırmadan sonra faturalandırma işlem tanımlayıcısı, mevcutsa |
...
<Card
  ...
  href="/openapi.json"
>
  Tam OpenAPI 3.1 spesifikasyonunu indir
</Card>
