...
description: "Créer une tâche Seedance 2.0 avec l'API compatible Volc."
---
...
- Le corps de la requête n'accepte que les champs pris en charge par ce format. Les champs de premier niveau `material_asset_id`, `material_asset_ids` et `priority` sont rejetés. Insérez plutôt un URI de matériel TokenLab dans `image_url.url`.

## Notes sur les paramètres

- `ratio` accepte `16:9`, `4:3`, `1:1`, `3:4`, `9:16`, `21:9` ou `adaptive`.
- `duration` est la durée de la vidéo en secondes, incluant les valeurs de durée automatique prises en charge par le modèle.
- `resolution` accepte `480p`, `720p` ou `1080p`.
- `generate_audio` est défini par défaut sur `false` ; réglez-le sur `true` lorsqu'un audio synchronisé est requis.
...
Lorsque `content[]` contient des URL d'images, TokenLab les prépare en tant que matériel réutilisable lorsque cela est requis par le modèle Seedance sélectionné. Si la préparation n'est pas terminée dans les 60 secondes, la requête de création renvoie une erreur de préparation de matériel réessayable au lieu de soumettre une tâche vidéo incomplète.
...
- Si la requête originale est toujours en cours d'enregistrement, TokenLab renvoie `409 IdempotencyRequestInProgress` avec `Retry-After` ; réessayez avec la même clé et le même corps après ce délai.
...
Utilisez l'ID `cgt-...` renvoyé avec [Obtenir une tâche (Compatible Volc)](/api-reference/video/get-volc-compatible-seedance-task) jusqu'à ce que la tâche atteigne un statut terminal.