...
`GET /v1/models`, `GET /v1/models/{model}` et `GET /v1/pricing` sont publics et ne nécessitent pas de clé. Anthropic Messages accepte aussi `x-api-key` ; Gemini accepte `x-goog-api-key` ou `?key=` en plus de Bearer. `/v1/management/*` exige un jeton de gestion (`mt-...`).
