...
description: "Configurer un modèle TokenLab dans Harness, vérifier une requête et choisir MCP, Skills ou le bundle de fournisseur"
---
...
Harness est une préversion destinée aux développeurs. Ces étapes pour l’interface Web ont été vérifiées le 16 septembre 2026 à partir de la documentation officielle et du paquet publié `@deepseek-ai/dsh` `0.1.5-rc.1`. Les instructions du bundle ci-dessous ciblent aussi cette version ; vérifiez séparément la compatibilité avec les autres versions de Harness.
...
[`@tokenlabai/dsh-provider@0.1.3`](https://www.npmjs.com/package/@tokenlabai/dsh-provider/v/0.1.3) cible Harness `0.1.5-rc.1`. Utilisez les étapes natives ci-dessus pour configurer un seul modèle, ou installez le bundle pour ses routes de modèles et ses outils préconfigurés.

Le bundle contient un **instantané fixe de 124 modèles de chat publics, vérifié le 16 septembre 2026** : Responses 23, Messages 9 et Chat 92. Chaque modèle figure sur une seule route. Il utilise la version fixe `@tokenlabai/mcp-server@0.6.19` et inclut l’outil distinct `tokenlab_wait_task`. L’installation de cette version n’actualise pas le catalogue. Vérifiez les identifiants dans le catalogue actuel et ajoutez les modèles plus récents via un custom provider si nécessaire.
...
- **`UNKNOWN_MODEL` ou un identifiant retiré :** consultez le catalogue actuel, configurez l'identifiant exact disponible, puis créez une nouvelle session. Réinstaller le bundle `0.1.3` n'actualise pas son instantané.
