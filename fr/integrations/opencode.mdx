---
title: "OpenCode"
description: "Configurer OpenCode pour utiliser l'API TokenLab"
---

## Vue d’ensemble

OpenCode est un assistant de codage IA open-source qui s’exécute dans votre terminal. Il prend en charge plusieurs fournisseurs de LLM et peut être configuré pour utiliser l’API de TokenLab afin d’accéder à plus de 300 modèles.

## Installation

[OpenCode](https://opencode.ai/docs/)

```bash
npm install -g opencode-ai
```

Vérifiez l’installation :

```bash
opencode --version
```

## Configuration

### Étape 1 : Définir les variables d’environnement

```bash
export TOKENLAB_API_KEY="sk-your-tokenlab-key"
```

Pour une configuration permanente, ajoutez ceci à `~/.bashrc` ou `~/.zshrc` :

```bash
echo 'export TOKENLAB_API_KEY="sk-your-tokenlab-key"' >> ~/.zshrc
source ~/.zshrc
```

### Étape 2 : Configurer OpenCode

La documentation actuelle d’OpenCode recommande de configurer les fournisseurs via `opencode.json` ou `opencode.jsonc`. Pour TokenLab, utilisez un fournisseur personnalisé au lieu de surcharger le fournisseur `openai` intégré :

Cet exemple utilise le format V1 pris en charge. Fusionnez-le dans `~/.config/opencode/opencode.json` en gardant fournisseurs et autorisations. V2 lit les champs V1 compatibles ; ne mélangez pas les formats dans un fournisseur. Voir la [migration officielle](https://opencode.ai/v2/docs/migrate-v1/). Vérifiez `/models`, puis comparez un test payant aux requêtes TokenLab. Pour annuler, revenez au modèle précédent et retirez cette entrée uniquement.

```jsonc
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "tokenlab": {
      "npm": "@ai-sdk/openai-compatible",
      "name": "TokenLab",
      "options": {
        "baseURL": "https://api.tokenlab.sh/v1",
        "apiKey": "{env:TOKENLAB_API_KEY}"
      },
      "models": {
        "gpt-5.6-terra": {
          "name": "GPT-5.6 Terra"
        },
        "gpt-5.6-luna": {
          "name": "GPT-5.6 Luna"
        }
      }
    }
  },
  "model": "tokenlab/gpt-5.6-terra"
}
```

<Note>
Utilisez `@ai-sdk/openai-compatible` pour une compatibilité de type chat-completions. Si vous avez spécifiquement besoin d’un chemin de fournisseur construit sur `/v1/responses`, la documentation des fournisseurs d’OpenCode recommande de remplacer le package par `@ai-sdk/openai`.
</Note>

## Utilisation de base

**Démarrer le mode interactif :**

```bash
opencode
```

**Exécuter avec un prompt :**

```bash
opencode run "Explain this codebase"
```

**Spécifier un modèle :**

```bash
opencode run --model tokenlab/gpt-5.6-terra "Fix the bugs in main.py"
```

## Modèles disponibles

| Modèle | Idéal pour |
|-------|----------|
| `gpt-5.6-terra` | Tâches complexes, architecture du code |
| `gpt-5.6-luna` | Corrections rapides, requêtes simples |
| `claude-sonnet-4-6` | Revue de code, documentation |
| `claude-opus-4-6` | Raisonnement complexe |
| `gemini-3.5-flash` | Réponses rapides |

## Commandes courantes

**Analyser du code :**

```bash
opencode run "What does this function do?" < src/utils.ts
```

**Générer du code :**

```bash
opencode run "Create a REST API with Express"
```

**Examiner les modifications :**

```bash
git diff | opencode run "Review these changes"
```

**Corriger les erreurs :**

```bash
opencode run "Fix the TypeScript errors in this project"
```

## Commandes interactives

| Commande | Description |
|---------|-------------|
| `/help` | Afficher les commandes disponibles |
| `/models` | Basculer vers un autre modèle |
| `/clear` | Effacer l’historique de la conversation |
| `/exit` | Quitter OpenCode |

## Dépannage

<AccordionGroup>
  <Accordion title="Erreur de connexion">
    - Vérifiez que `options.baseURL` est défini sur `https://api.tokenlab.sh/v1`
    - Vérifiez la connectivité réseau
    - Essayez `curl https://api.tokenlab.sh/v1/models` pour tester
  </Accordion>

  <Accordion title="Échec de l’authentification">
    - Vérifiez que la variable d’environnement `TOKENLAB_API_KEY` est définie
    - Vérifiez que `options.apiKey` référence `{env:TOKENLAB_API_KEY}` ou une autre source secrète valide
    - Vérifiez que la clé commence par `sk-`
    - Assurez-vous que la clé est active dans le tableau de bord TokenLab
  </Accordion>

  <Accordion title="Modèle introuvable">
    - Vérifiez que le nom du modèle dans la configuration correspond exactement
    - Vérifiez la disponibilité du modèle sur [tokenlab.sh/en/models](https://tokenlab.sh/fr/models)
  </Accordion>
</AccordionGroup>

## Bonnes pratiques

<AccordionGroup>
  <Accordion title="Utiliser le contexte du projet">
    Exécutez OpenCode depuis la racine de votre projet pour une meilleure compréhension de votre base de code.
  </Accordion>

  <Accordion title="Choisir des modèles appropriés">
    Utilisez des modèles plus rapides (`gpt-5.6-luna`) pour les tâches simples et des modèles plus puissants (`gpt-5.6-terra`, `claude-sonnet-4-6`) pour les tâches complexes.
  </Accordion>

  <Accordion title="Relire le code généré">
    Relisez toujours le code généré par l’IA avant d’appliquer des modifications à votre projet.
  </Accordion>
</AccordionGroup>