...
Pour tout client stdio local, utilisez le nom `tokenlab`, la commande `npx`, les arguments séparés `-y` et `@tokenlabai/mcp-server`, et aucun répertoire de travail. Utilisez `@tokenlabai/mcp-server@0.6.19` pour figer la version actuellement vérifiée.
