...
- MCP ajoute des outils API TokenLab aux clients compatibles. Commencez ci-dessous avec `catalog`, sans clé API.
- Le [Skill TokenLab](/fr/integrations/coding-agent-skill) installe des instructions avec `npx skills add` sans lancer de serveur MCP.

Ces extensions complètent un agent existant. Pour changer son fournisseur principal, suivez le guide du client.
...
```text
Lis ce guide et choisis les outils de catalogue MCP ou le Skill TokenLab selon ma tâche :
https://docs.tokenlab.sh/fr/integrations/tokenlab-mcp-server
...
<CardGroup cols={2}>
  <Card title="Installer dans Cursor" icon="arrow-pointer" href="https://cursor.com/en/install-mcp?name=tokenlab&config=eyJ0eXBlIjoic3RkaW8iLCJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsIkB0b2tlbmxhYmFpL21jcC1zZXJ2ZXJAMC42LjE5Il0sImVudiI6eyJUT0tFTkxBQl9NQ1BfVE9PTF9QUk9GSUxFIjoiY2F0YWxvZyJ9fQ%3D%3D">
    Ajouter le profil public `catalog` à Cursor en un clic
  </Card>
  <Card title="Installer dans VS Code" icon="code" href="https://vscode.dev/redirect/mcp/install?name=tokenlab&config=%7B%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40tokenlabai%2Fmcp-server%400.6.19%22%5D%2C%22env%22%3A%7B%22TOKENLAB_MCP_TOOL_PROFILE%22%3A%22catalog%22%7D%7D">
...
<Tabs>
      ...
      <Tab title="Claude Desktop">
        Dans `claude_desktop_config.json`, ajoutez à l'objet standard `mcpServers` la commande `npx` et les arguments `-y` et `@tokenlabai/mcp-server@0.6.19`. Pour les seuls outils publics, définissez `TOKENLAB_MCP_TOOL_PROFILE=catalog`.
      </Tab>
...
Pour tout client stdio local, utilisez le nom `tokenlab`, la commande `npx`, les arguments séparés `-y` et `@tokenlabai/mcp-server@0.6.19`, et aucun répertoire de travail. Utilisez `@tokenlabai/mcp-server@0.6.19` pour figer la version actuellement vérifiée.
...
<AccordionGroup>
  <Accordion title="Le client ne peut pas démarrer le serveur">
    Vérifiez que Node.js est au moins en version 18.17 et que le client trouve `npx`. Exécutez `npx -y @tokenlabai/mcp-server@0.6.19` dans un terminal pour afficher les erreurs npm ou réseau. Un démarrage stdio réussi attend silencieusement les messages MCP ; quittez avec **Ctrl+C** après la vérification.
  </Accordion>
  ...
  <Accordion title="Comment figer ou mettre à jour la version du paquet ?">
    Utilisez `@tokenlabai/mcp-server@0.6.19` pour figer la version actuellement vérifiée. Pour changer de version, modifiez l'argument du paquet, redémarrez le serveur et consultez le dépôt public avant d'adopter une version plus récente.
  </Accordion>
</AccordionGroup>
