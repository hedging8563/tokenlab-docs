...
The material library applies a 30-day inactivity cleanup: an asset that was uploaded 30 or more days ago and has not been referenced by any generation request in the past 30 days is deleted automatically. Deletion is permanent and cannot be undone.

- Referencing an asset ID in a generation request (`material_asset_id`, `material_asset_ids`, or `asset://<id>`) counts as usage and keeps the asset from being cleaned up.
- Back up important source files in advance. If an asset is cleaned up, import it again; the reimported asset receives a new asset ID.
...
The source URL is only used for import. TokenLab returns its own asset ID and keeps the asset in your organization's material library until you delete the asset or its group. Assets that remain inactive are also cleaned up automatically under the material retention rule above.
