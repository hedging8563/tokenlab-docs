...
Seedance can turn supported image inputs into reusable materials automatically. If an image is still being prepared after 60 seconds, the API returns `auto_material_asset_ids`; wait until those materials are `ACTIVE`, then send the request again.
