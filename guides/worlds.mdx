...
A world can start from text, one or more images, or video, depending on the model. Reusable media can be uploaded once and referenced by its asset ID.

Access to media and world records is checked against the organization that owns the API key. Review the selected model’s `permission` options before sharing a world; a returned asset or viewer URL may have its own access rules.
