...
A biblioteca de materiais aplica uma limpeza por inatividade de 30 dias: um ativo enviado há 30 dias ou mais e que não tenha sido referenciado por nenhuma solicitação de geração nos últimos 30 dias é excluído automaticamente. A exclusão é permanente e não pode ser desfeita.

- Referenciar o ID de um ativo em uma solicitação de geração (`material_asset_id`, `material_asset_ids` ou `asset://<id>`) conta como uso e protege o ativo dessa limpeza.
- Faça backup dos arquivos de origem importantes com antecedência. Se um ativo for removido, importe-o novamente; o ativo reimportado recebe um novo ID.
...
A TokenLab mantém os ativos de material na biblioteca da sua organização até que você exclua o ativo ou seu grupo de materiais. Ativos que permanecem inativos também são removidos automaticamente conforme a regra de retenção de materiais acima.
