...
description: "Configure um modelo TokenLab no Harness, verifique uma solicitação e escolha MCP, Skills ou o bundle de provedor publicado"
---
...
O Harness é uma versão de prévia para desenvolvedores. Estas etapas da interface web foram conferidas com a documentação oficial e o pacote publicado `@deepseek-ai/dsh` `0.1.5-rc.1` em 16 de setembro de 2026. Isso não significa que o bundle anterior do TokenLab funcione com todas as versões `0.1.x`.
...
## Opcional: o bundle publicado do TokenLab

[`@tokenlabai/dsh-provider@0.1.1`](https://www.npmjs.com/package/@tokenlabai/dsh-provider/v/0.1.1) está publicado. Ele é destinado ao Harness `0.1.1-rc.2`; a compatibilidade com versões mais recentes do Harness precisa ser conferida separadamente. Para uma nova configuração, prefira as etapas nativas de custom provider descritas acima.

Esse bundle contém um **snapshot fixo dos modelos de 26 de agosto de 2026**, três rotas de provedor, `@tokenlabai/mcp-server@0.6.17` com as 80 ferramentas do perfil `full` e `tokenlab_wait_task`. Instalar essa versão não atualiza o catálogo de modelos. Confira cada ID no catálogo atual; adicione modelos mais recentes por meio de um custom provider, em vez de presumir que o bundle já os inclui.
...
Reinicie o mesmo perfil e confira suas listas de modelos e ferramentas. Para `headless`, instale e inicie usando `headless` em vez de `web`; instalar em um perfil não configura o outro. As configurações existentes de `llm-pi-ai` têm prioridade sobre os padrões do bundle. Preserve-as e mescle as entradas de provedor necessárias em `$DSH_HOME/settings.yaml`, em vez de substituir suas configurações pelo patch Cordis inteiro do bundle.

O bundle usa por padrão `TOKENLAB_MCP_TOOL_PROFILE=full` e `TOKENLAB_MCP_SCHEMA_MODE=portable`. Use `catalog` apenas para descoberta ou `core` para um conjunto menor de ferramentas. `TOKENLAB_API_BASE` e `TOKENLAB_ANTHROPIC_BASE_URL` têm como padrão `https://api.tokenlab.sh`; `TOKENLAB_OPENAI_BASE_URL` tem como padrão `https://api.tokenlab.sh/v1`.
...
- **`UNKNOWN_MODEL` ou um ID retirado:** confira o catálogo atual, configure o ID exato disponível e crie uma nova sessão. Reinstalar o bundle `0.1.1` não atualiza seu snapshot.
