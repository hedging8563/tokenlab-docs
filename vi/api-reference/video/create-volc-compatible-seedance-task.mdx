...
openapi: "/openapi/vi.json POST /api/v3/contents/generations/tasks"
description: "Tạo tác vụ Seedance với API tương thích Volc."
---
...
- Các trường cấp cao nhất `material_asset_id` và `material_asset_ids` bị từ chối. Đặt URI tài nguyên TokenLab vào `image_url.url`. Chỉ Seedance 2.5 hỗ trợ `priority`.
...
- Nếu yêu cầu gốc vẫn đang được đăng ký, TokenLab trả về `409 IdempotencyRequestInProgress`. Thử lại sau với cùng khóa và phần thân.
...
Sử dụng ID `cgt-...` được trả về với [Lấy tác vụ (Tương thích Volc)](/vi/api-reference/video/get-volc-compatible-seedance-task) cho đến khi tác vụ đạt trạng thái cuối cùng.