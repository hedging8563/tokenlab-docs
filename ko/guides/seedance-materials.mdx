...
소재 라이브러리에는 30일 비활성 정리 규칙이 적용됩니다. 업로드 후 30일 이상 경과하고 지난 30일 동안 어떤 생성 요청에서도 참조되지 않은 소재는 자동으로 삭제됩니다. 삭제 후에는 복구할 수 없습니다.

- 생성 요청에서 소재 ID(`material_asset_id`, `material_asset_ids` 또는 `asset://<id>`)를 참조하면 사용한 것으로 간주되어 해당 소재는 이 규칙으로 삭제되지 않습니다.
- 중요한 소재는 원본 파일을 미리 백업해 두세요. 삭제된 소재를 다시 사용하려면 다시 가져와야 하며 새 소재 ID가 발급됩니다.
...
TokenLab은 소재 또는 해당 소재 그룹을 삭제할 때까지 조직 소재 라이브러리에 소재를 보관합니다. 오래 참조되지 않은 소재는 위의 「소재 보관 기간」 규칙에 따라 자동으로 삭제됩니다.
