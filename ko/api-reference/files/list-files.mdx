---
title: "파일 목록"
openapi: "/openapi/ko.json GET /v1/files"
...
- 배치 및 이미지 참조 파일의 `limit`는 1–10000(기본값 10000)이며 `after`로 페이지를 넘깁니다. Anthropic Files 모드에서는 1–1000(기본값 20)입니다. 페이지 이동에는 `after_id` 또는 `before_id` 중 하나만 사용하세요.
- `purpose`는 `batch`, `batch_output`, `user_data`, `vision` 중 하나일 수 있습니다.
- `user_data`와 `vision`은 사용 가능한 경우 TokenLab이 생성한 이미지 참조 파일을 반환합니다.
