---
title: "파일 내용 가져오기"
openapi: "/openapi/ko.json GET /v1/files/{file_id}/content"
...
배치 파일 콘텐츠는 JSONL 또는 일반 텍스트로 반환됩니다. TokenLab이 생성한 이미지 참조 콘텐츠는 백킹 파일 제공자가 콘텐츠 조회를 지원하는 경우 바이너리 콘텐츠로 프록시될 수 있습니다.
...
<ResponseField name="body" type="binary|string">파일 내용입니다. `Content-Type`에 따라 저장하거나 파싱하세요. JSONL은 한 줄씩 파싱합니다.</ResponseField>
<ResponseField name="custom_id" type="string">배치 입력 항목과 결과를 연결하는 ID입니다.</ResponseField>
