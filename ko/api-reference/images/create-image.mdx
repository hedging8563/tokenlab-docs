---
title: "이미지 생성"
openapi: "/openapi/ko.json POST /v1/images/generations"
...
- `nano-banana-2`는 text-to-image와 image-to-image 모두에서 `aspect_ratio`와 `resolution` (`1k`, `2k`, `4k`)을 지원합니다.
...
<ParamField body="resolution" type="string">
  출력 해상도는 모델에 따라 다릅니다. `gemini-3.1-flash-image`, `gemini-3-pro-image`, `nano-banana-2`, `nano-banana-pro`는 `1k`, `2k`, `4k`를 지원하며 Grok Imagine 이미지 모델은 `1k`, `2k`를 지원합니다. 선택한 모델과 작업이 명시적으로 지원할 때만 보내세요.
</ParamField>
...
현재 이미지 모델, 기능, 가격은 `GET /v1/models?recommended_for=image`에서 확인하세요.
